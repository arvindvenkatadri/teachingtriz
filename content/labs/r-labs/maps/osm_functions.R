...
coords <- matrix(c(-0.1,-0.07,51.5,51.52), byrow = TRUE, 
                 nrow = 2, ncol = 2, 
                 dimnames = list(c('x','y'),c('min','max')))
location <- coords %>% opq()

#create different types of streets
main_st <- data.frame(type = c("motorway","trunk","primary",
                               "motorway_junction","trunk_link",
...
st <- subset(st, !type.Key %in% main_st)
st <- subset(st, !type.Value %in% path)
st <- as.character(st$type.Value)
