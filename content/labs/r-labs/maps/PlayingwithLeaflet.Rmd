...
NOTE: the `radius` argument works differently in `addCircles()` and
`addCircleMarkers()`.
...
on a `leaflet` map. `leaflet` takes in geographical data in many ways
and we will explore most of them.

## Point Data Sources for `leaflet`

Point data for markers can come from a variety of sources:

-   `SpatialPoints` or `SpatialPointsDataFrame` objects (from the `sp`
    package)\
-   `POINT`, `sfc_POINT`, and `sf` objects (from the `sf` package); only
    X and Y dimensions will be considered\
-   Two-column numeric matrices (first column is `longitude`, second is
    `latitude`)\
-   `Data frame/tibble` with `latitude` and `longitude` columns. You can
    explicitly tell the marker function which columns contain the
    coordinate data (e.g.
...
-   Simply provide numeric `vectors` as `lng` and `lat` arguments, which
    we have covered already in the preceding sections.
...
<https://query.data.world/s/ahtyvnm2ybylf65syp4rsb5tulxe6a>. You can
either download it, save a copy, and read it in as usual, or use the URL
itself to read it in from the web. In the latter case, you will need the
package `data.world` and also need to register your credentials for that
page with RStudio. The (simple!) instructions are available here at
...
theseveral icon families that we can use with `leaflet` : glyphicons,
ionicons, and fontawesome icons.
...
In `sf`, the `lon` and `lat` info is embedded in the `geometry` column
of the `sf data frame`.
...
say OSM data <https://www.openstreetmap.org/#map=16/12.9766/77.5888>. Let us get hold of restaurants data in Malleswaram, Bangalore from OSM data:
...
fontawesome icons simply do not seem to show up. ;-() ( Update Dec 2023: Seems OK now...)
...
-   `SpatialPolygons`, `SpatialPolygonsDataFrame`, `Polygons`, and
    `Polygon objects` (from the `sp` package)\
-   `SpatialLines`, `SpatialLinesDataFrame`, `Lines`, and `Line objects`
    (from the `sp` package)\
-   `MULTIPOLYGON`, `POLYGON`, `MULTILINESTRING`, and `LINESTRING`
    objects (from the `sf` package)\
-   `map` objects (from the `maps` package's `map()` function); use
    `map(fill = TRUE)` for polygons, `FALSE` for polylines\
-   Two-column numeric `matrix`; the first column is longitude and the
    second is latitude. Polygons are separated by rows of (NA, NA). It
    is not possible to represent multi-polygons nor polygons with holes
    using this method; use `SpatialPolygons` instead.
...
We have `r nrow(colleges)` colleges, `r nrow(parks)` parks, `r nrow(roads)` roads, and `r nrow(cyclelanes)` cyclelanes
in our data.
...
**vector** data. We will now explore how to plot **raster** data using
`leaflet`. Raster data are used to depict continuous variables across
space, such as vegitation, salinity, forest cover etc. Satellite imagery
is frequently available as raster data.
