...
<a href="../../../../materials/data/tidy/artworks-cleaned.csv">
<i class="fa-solid fa-database"></i> artworks-cleaned.csv</a>

```{r, echo=FALSE}
moma <- read_csv("../../../materials/data/tidy/artworks-cleaned.csv",show_col_types = FALSE)
inspect(moma)
...
`r pablo %>% pull(artist)` has `r pablo %>% pull(n)` paintings in the
MoMA collection. But how can we find out which female artist has the
most paintings? We have a few options. Let's first add a `filter` for
females.
...
Now we can see that `r sherrie %>% pull(artist)` has
`r sherrie %>% pull(n)` paintings. This is a pretty far cry from the
`r pablo %>% pull(n)` paintings by `r pablo %>% pull(artist)`.
