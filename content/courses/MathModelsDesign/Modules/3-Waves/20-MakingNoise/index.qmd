...
![](../../../../../materials/images/Bheeshma.jpeg)
:::
...
So how we create and merge overlays? We create [several more-closely-spaced grids overlaid on the first one]{.black .bg-pink}, and generate noise in the same way. These layers of noise-s are scaled by a factor (Usually $\Large{\frac{1}{2^n}}$), where $n$ is the "order" of the layer. Each new finely-spaced layer generates similar-looking noise functions, which are combined with smaller and smaller weights to achieve that final polished fractal look of Perlin Noise. 
