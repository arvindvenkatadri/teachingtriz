...
<https://github.com/stla/PlaneGeometry/blob/master/vignettes/examples.Rmd>
...
```{r tessellation, fig.width=4, fig.height=4, eval=require("viridisLite", quietly=TRUE)}
tessellation(
  depth = 5L, 
  Thetas0 = c(0, 2, 3.8), 
...
```{r tessellation2, fig.width=4, fig.height=4, eval=require("viridisLite", quietly=TRUE)}
tessellation2(
  depth = 5L, 
  Thetas0 = c(0, 2, 3.8), 
...
```{r schottky, message=FALSE, eval=require("freegroup", quietly=TRUE)}
library(freegroup)
...
```{r draw_schottky, fig.width=4, fig.height=4, eval=require("freegroup", quietly=TRUE)}
opar <- par(mar = c(0,0,0,0), bg = "black")
...
```{r modularTesselation, message=FALSE, eval=require("elliptic", quietly=TRUE)}
...
```{r drawgasket, fig.height=4, fig.width=4, eval=require("viridisLite", quietly=TRUE)}
...
```{r rglsphere, eval=require("rgl", quietly=TRUE)}
library(rgl)
...
```{r malfattigasket, eval=require("viridisLite", quietly=TRUE)}
toCplx <- function(M) {
  M[1L] + 1i * M[2L]
}
fromCplx <- function(z) {
...
```{r malfattiCircular, warning=FALSE, eval=require("rgl", quietly=TRUE)}
library(rgl)
...
```{r plotMalfattiCircularGasket, eval=FALSE}
...
```{r hyperbola, fig.width=7, fig.height=5}
# take a hyperbola
