...
data <- tibble::tibble(t = seq(0, 2*pi, 0.001),
...
$2\pi/5$ radians. [Because the frequencies 1, 6, and −14 are all congruent to 1 modulo 5]{.bg-pink .black}, shifting time by $2\pi/5$ causes the equation to add on a complex rotation term of $e^{2\pi*i/5}$. Time shifts are Angle Shifts. 
...
# ## Coefficients 1, 1, 1
data4 <- tibble::tibble(t = seq(0, 2*pi, 0.001),
          x = cos((0 + 2)*t) + cos((5 + 2)*t) + sin(-(-15 + 2)*t),
          y = sin((0 + 2)*t) + sin((5 + 2) *t) + cos(-(-15 + 2)*t)
)
...
data5 <- tibble::tibble(t = seq(0, 2*pi, 0.001),
          x = cos((0 + 2)*t) - cos((5 + 2)*t)/2 - cos(-(15 - 2)*t)/3,
          y = sin((0 + 2)*t) - sin((5 + 2) *t)/2 - sin(-(15 - 2)*t)/3
)
