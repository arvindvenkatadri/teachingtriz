...
-   What attributes (i.e. extra information) are seen for Nodes and
    Edges?
-   Understand the data in both nodes and edges as shown in the second
    and third thumbnails. Write some comments and inferences here.
:::
...
-   What parameters have been changed here, compared to the earlier
    graph?
...
-   How does this graph look "metaphorically" different?
-   Do you see a difference in the relationships between people here?
-   If so, Why?
-   If not, why not?
:::
...
ggraph(ga,layout = "linear", circular = TRUE) + 
  ...
  geom_node_point() +
  facet_edges(~ type) + theme(aspect.ratio = 1)

# facet nodes by sex
ggraph(ga,layout = "linear", circular = TRUE) + 
  geom_edge_link() + 
  geom_node_point() +
  facet_nodes(~race)+ theme(aspect.ratio = 1)
...
ggraph(ga,layout = "linear", circular = TRUE) + 
  ...
  th_foreground(border = TRUE)+ theme(aspect.ratio = 1)
```

::: {#QI-10 .callout-note}
## Questions and Inferences #10:

-   Does splitting up the main graph into subnetworks give you more
    insight?
...
![Standards](standards.png){fig-alt="XKCD comic showing the
proliferation of Network Centrality Measures" fig-align="center"}
...
-   Try to modify the aesthetics by copy-pasting this chunk below and
    see how you can make an alternative representation.
:::
