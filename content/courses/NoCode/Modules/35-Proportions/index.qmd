...
We saw with [Bar Charts](../20-BarCharts/index.qmd) that when we deal with single Qual variables, we perform *counts* for each *level* of the variable. 

What if there are two Quals? Or even more?

The answer is to take them pair-wise, make all combinations of *levels* for both and calculate counts for these. This is called a **Contingency Table**. 
...
Ok, let us see if we can rescue Jack also. Here is the `titanic` data. 
...
This dataset was generated as part of a landmark study done by [Marianne Bertrand and Senthil Mullainathan](https://sci-hub.se/10.1257/0002828042002561). Read the 
