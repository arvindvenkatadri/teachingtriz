...
Note: `group_modify` is new **experimental** function in `dplyr`, as are
`group_map`, `list_cbind` and `list_rbind`. `group_modify` **requires**
...
column\
- Use purrr::map\_\* to create a model for each of these data frames in
