...
```
Note that this is still *just a tibble*, with a time-formatted column.
Next let us create a full-blown `tsibble` with the same data:
:::
:::{.column}
```{r}
#| ref.label: making-time-column
#| echo: false
```
...
## {{< iconify mdi chart-line >}} Basic Time Series Plot
...
## {{< iconify carbon chart-average >}} Aggregation and Averaging 
...
:::{.column}
```{r}
#| ref.label: Per-month-aggregation
#| echo: false
```
...
trends** that repeat each year, which we glimpsed earlier. We will analyse seasonal trends in another module. 
...
### {{< iconify game-icons detour >}} A small detour{.hidden}
...
:::{.column width="40%"}
```{r}
#| ref.label: Basic-PBS-plot-ggformula
#| echo: false
```
...
:::{.column width="40%"}
```{r}
#| ref.label: PBS-Costs-filtered-ggformula
#| echo: false
```
...
```{r}
#| ref.label: Summary-over-months-with-timetk
#| echo: false
```
...
```{r}
#| label: Summary-over-quarters-with-timetk
#| eval: false
#| message: false
# Set graph theme
theme_set(new = theme_custom())
...
```{r}
#| ref.label: Summary-over-quarters-with-timetk
#| echo: false
```
...
```{r}
#| label: Summary-over-years-with-timetk
#| eval: false
# Set graph theme
theme_set(new = theme_custom())
PBS %>% 
...
```{r}
#| ref.label: Summary-over-years-with-timetk
#| echo: false
```
...
1.  Choose some of the data sets in the `tsdl` and in the `tsibbledata`
