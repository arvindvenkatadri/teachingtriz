...
varying in accordance to the data variable**]{style="background: yellow;"}. 

For instance, `length` or `height` of a `bar` can be made proportional 
to the`age` or `income` of a person. `Colour` of points can be mapped 
to `gender`, with a unique `colour` for each `gender`. `Position` along 
an axis x can vary in accordance with a `height` variable and position 
