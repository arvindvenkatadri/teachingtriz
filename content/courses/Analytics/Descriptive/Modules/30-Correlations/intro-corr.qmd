...
```{r}
#| label: corr-ggplot-scores
#| eval: false
movies %>% drop_na() %>%
  ggplot(aes(AudienceScore, RottenTomatoes, colour = Genre)) + 
  geom_point() + 
  geom_lm()

```

By and large yes..there are only small variations in slope across `Genre`s. 
:::
...
```{r}
#| column: body-outset-right
mosaic::cor_test(Budget ~ Profitability, data = movies) %>% 
  broom::tidy() %>% 
  knitr::kable(digits = 2,
               caption = "Movie Budget vs Profitability")
...
4. Antoine Soetewey. *Pearson, Spearman and Kendall correlation coefficients by hand* <https://www.r-bloggers.com/2023/09/pearson-spearman-and-kendall-correlation-coefficients-by-hand/>
