...
`dplyr` does for tibbles. We will do this shortly, but using `tsibble`
however.
:::
...
containing 204 observations (except some!): so let us filter for a few
such combinations and plot:
...
quarter, month(original data): Look carefully at the new time variable created each time:
