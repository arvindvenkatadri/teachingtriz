...
have **multiple** **time series** within a tibble (think GDP,
Population, Imports, Exports for multiple countries as with the
`gapminder`[^1]data we saw earlier). It also allows for data processing
with `dplyr` such as filtering and summarizing.
...
we have considerable variation over time. Summaries will help, so we could calculate the the **mean births on a month basis** in each
year and plot that:
...
create a single `date` column from these three, but retain them for now. We use the `lubridate` package from the `tidyverse`:
...
(`Scripts`,`Cost`), each representing a timeseries, a feature which is not supported in the `ts` format, but is supported in a `tsibble`. The **Qualitative Variables** are described below. [Type `help("PBS")` in your Console.]{.aside}
...
We **can** use `dplyr` functions such as `mutate()`, `filter()`,
`select()` and `summarise()` to work with `tsibble` objects. `tsibble`
does not allow filtering based on categorical variables, that needs to
be done with `dplyr`.

However, `tsibble` has specialized functions to do with the `index` (i.e
time) variable and the `key` variables, things similar to what `dplyr`
does.
:::
...
containing 204 observations (except some! Take a look!): so let us filter for a few such combinations and plot:
...
payment methods, and also with `Costs` and `Scripts`. Strongly seasonal for both, with seasonal variation increasing over the years; stronger upward *trend* with the `Co-payments` method of payment for the `Costs` time series, but with `Safety Net` for the `Scripts` time series.
...
8.  [Visualizing Statistical Mix Effects and Simpson's Paradox](https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/42901.pdf)

9.  [How To Fix a Toilet (And Other Things We Couldn't Do Without Search)](http://how-to-fix-a-toilet.com/)
