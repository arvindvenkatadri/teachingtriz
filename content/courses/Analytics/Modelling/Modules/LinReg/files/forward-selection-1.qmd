...
improvement offered by each predictor *individually*, we will add another quantitative predictor. We will follow this process through up to a point 
where the gains in model accuracy are good enough to justify the additional model complexity.
...
We ought to convert the above code into an R `function` and run it that way for a specific number of rounds to see how things pan out. That is in the next
