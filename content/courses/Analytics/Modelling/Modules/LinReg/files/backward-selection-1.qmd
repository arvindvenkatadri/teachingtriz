...
terms* : each predictor was used only for its **main effect**. When the diagnostic plots were examined, we did see some shortcomings in the model. This could be overcome with a more complex model. These might include selected interactions, transformations of target($cmedv^2$, or $sqrt(cmedv)$) and some selected predictors.
