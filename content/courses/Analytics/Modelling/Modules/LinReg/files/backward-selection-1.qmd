...
all_corrs %>% 
  ...
  gf_labs(x = NULL, y = "Correlation with Median House Price") %>% 
  gf_hline(yintercept = 0, color = "grey", linewidth = 2) %>% 
  gf_theme(theme_minimal()) %>% 
  gf_refine(scale_colour_gradient("significance", low = "grey", high = "red"))
...
The last 5 variables are insignificant and almost contribute nothing to
the `r.squared` value. We can understand that only after we drop the
10th and more significant variables, that we might see significant drop
in `r.squared`.
...
We will use the iteration capability of `purrr` to sequentially drop one
variable at a time from the maximal(15 predictor) model, build a new
reduced model each time, and compute the `r.squared`:
...
$$
\widehat{cmedv} \sim 24.459 - 0.673 * lstat + 4.199 * rm - 0.957 * ptratio - 0.563 * dis
...
However, the models we chose were all all linear and without interaction
terms. When the diagnostic plots were examined, we did see some
shortcomings in the model. This could be overcome with a more complex
model.
...
[^1]: James, Witten, Hastie, Tibshirani, \\\* An Introduction to
    Statistical Learning\\\*. Chapter 3. Linear Regression
    \<https://hastie.su.domains/ISLR2/Labs/Rmarkdown_Notebooks/Ch3-linreg-lab.html\>
