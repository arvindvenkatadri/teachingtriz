...
|                                                                                                                  |     |                                                                                                             |     |
|------------------|-------------------|------------------|------------------|
| <a href="./files/multiple-regression.qmd"><i class="fa-brands        fa-r-project"></i>Multiple Regression</a>   |     | <a href="./files/lin-perm.qmd"> <i class="fa-brands fa-r-project"></i> Permutation Test for Regression</a>  |     |
...
will first do this with `GGally`, and then with the `tidyverse` itself. Both give us a very unique view into the correlations that exist within this dataset. 
...
Recall that `cor.test` reports a correlation score and the `p-value`
for the same. There is also a `confidence interval` reported for the
correlation score, an interval within which we are 95% sure that the true
correlation value is to be found.  Note that GGally too reports the significance of the correlation scores using `***` or `**`. This indicates the p-value in the scores obtained by GGally; Presumably, there is an internal `cor.test` that is run for each pair of variables and the p-value and confidence levels are also computed internally. 
:::
...
advantage of being able to depict all correlations in one plot. Let us
do this.
...
all_corrs <- housing %>% 
  ...
  select(-cmedv) %>%  # leave off cmedv and IDs to get all the remaining ones
  purrr::map(.x = .,
...
all_corrs %>% gf_point(estimate ~ reorder(predictor, estimate)) %>% gf_errorbar(conf.high + conf.low ~ reorder(predictor, estimate), width = 0.2) %>% 
...
housing_lm_tidy <- housing_lm %>% 
  broom::tidy(conf.int= TRUE, 
              conf.level = 0.95)
housing_lm_tidy

housing_lm_augment <- housing_lm %>% 
