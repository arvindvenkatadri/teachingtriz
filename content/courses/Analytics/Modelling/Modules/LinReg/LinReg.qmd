...
my_theme <- function(){  # Creating a function
  theme_light() +  # Using pre-defined theme as base
  theme(text = element_text(family = "Merri"),
        plot.title = element_text(size = 16, face = "bold"),
        plot.subtitle = element_text(size = 12, face = "bold"),
        axis.text.x = element_text(size = 8, face = "bold"),  # Customizing axes text      
        ...
        plot.caption = element_text(size = 10))  # Customizing plot caption
}                                                                         
...
![Least Squares](../../../../../materials/images/OLS.png)
...
![OLS Assumptions](../../../../../materials/images/ols_assumptions.png)
...
::: {.callout-important style="background: beige"}
...
housing %>% 
  ...
  GGally::ggpairs(title = "Plot 1", 
                  lower = list(continuous = wrap("smooth", alpha = 0.2))) + my_theme()
...
# Target variable and Access to Radial Highways, / Resid. Land Proportion / proportion of non-retail business acres / full-value property-tax rate per USD 10,000
  ...
  GGally::ggpairs(title = "Plot 2", lower = list(continuous = wrap("smooth", alpha = 0.2))) + my_theme()

housing %>% 
  # Target variable and Crime Rate / Nitrous Oxide / Black Population / Lower Status Population
  select(cmedv, crim, nox, rad, b, lstat) %>% 
  GGally::ggpairs(title = "Plot 3", lower = list(continuous = wrap("smooth", alpha = 0.2))) + my_theme()


housing %>% 
  # Target variable and Access to Charles River
  select(cmedv,chas) %>% 
  GGally::ggpairs(title = "Plot 4", lower = list(continuous = wrap("smooth", alpha = 0.2))) + my_theme()
...
Let us also check the Qualitative variables: Access to the Charles river
(`chas`) does seem to affect the prices somewhat.
...
significance of the correlation scores using stars, `***` or `**`. This
...
This is somewhat advanced material: We will use `purrr` to develop all
correlations with respect to our target variable in one shot and also
plot these correlation test scores in an error-bar plot. This has the
advantage of being able to depict all correlations in one plot. (We will
use this approach again here when we trim our linear models down from
the *maximal* one to a workable one of lesser complexity.). Let us do
this.
...
all_corrs %>% 
  gf_hline(yintercept = 0, color = "grey", linewidth = 2, title = "Correlations: Target Variable vs All Predictors", subtitle = "Boston Housing Dataset") %>% 
  ...
  gf_labs(x = NULL, y = "Correlation with cmedv") %>% 
  gf_theme(my_theme()) %>% 
  gf_refine(scale_colour_distiller("Correlation", type = "div", palette = "RdBu"),
...
Model is given by $474.3$, which is very high. ( We will use the
F-statistic again when we do Multiple Regression.) The `R-squared` value
is 48% which means that `rm` is able to explain about half of the trend
in `cmedv`; there is substantial variation in `cmedv` that is left to
...
predictions of `cmedv` for all *existing* values of `rm`. To predict
`cmedv` with *new* values of `rm`, we use `predict`. We see estimates
for the intercept and slope (`rm`) for the linear model, along with the
*standard errors* and *p.values* for these estimated parameters. And we
see the fitted values of `cmedv` for the existing `rm`; these values
will naturally lie **on** the straight-line depicting the model. We will
...
patchwork::wrap_plots(p1, p2) + patchwork::plot_annotation(tag_levels = c("A", "B"),tag_prefix = "Fig.")
...
sim_model %>% 
  ...
  gf_segment(min_r_squared + 0 ~ min_slope + min_slope, colour = "red", arrow = arrow(ends = "last", length = unit(1, "mm"))) %>%
  # gf_lims(y = c(-500, 20000)) %>%
  gf_theme(my_theme)
...
$$ ::: callout-tip Note that the slope is equal to the ratio of
:::
...
we will examine in the Tutorials in @sec-tutorials. Sometimes it may be
necessary to mathematically transform the variables in the dataset to
enable the construction of better models, something that was not needed
here. We may also encounter cases where the predictor variables seem to
work together; one predictor may influence "how well" another predictor
...
5.  David C Howell, [Permutation Tests for Factorial ANOVA
    Designs](https://www.uvm.edu/~statdhtx/StatPages/Permutation%20Anova/PermTestsAnova.html)
6.  Marti Anderson, [Permutation tests for univariate or multivariate
...
[^1]: The model is linear in the **parameters** $\beta_i$, e.g.
We can have this:
    $$
