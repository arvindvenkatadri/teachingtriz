...
```{mermaid}
%%| label: fig-regression-flowchart
%%| fig-cap: Linear Regression Workflow
%%| echo: false
%%| eval: false
...
We will do the first two in the other tutorials (see @sec-linreg-tutorials above); Mixed Selection we will leave for a more advanced course. But
...
-   The `R-squared` value is 48% which means that `rm` is able to
    ...
    explore this in the Tutorials. @sec-linreg-tutorials
:::
...
the NULL (empty) model, and the current model for comparison. The `PRE` entry is the
...
to the NULL model(48%). The PRE idea is nicely discussed in Judd et al.@sec-references
...
we will examine in the Tutorials in @sec-linreg-tutorials.
...
The Boston Housing Dataset, corrected version. StatLib \@ CMU,
...
Gareth James, Daniela Witten, Trevor Hastie, Robert Tibshirani,
    *Introduction to Statistical Learning*, Springer, 2021. Chapter 3. <https://www.statlearning.com/>\
