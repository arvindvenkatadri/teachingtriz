...
all_corrs %>% 
  ...
  gf_errorbar(conf.high + conf.low ~ reorder(predictor, estimate), 
              ...
              width = 0.2,caption = "Significance = -log10(p.value)") %>% 
  gf_labs(x = NULL, y = "Correlation with cmedv") %>% 
   gf_theme(theme_minimal()) %>% 
  gf_refine(scale_colour_gradient("significance", low = "blue", high = "red"))
...
more of them to make our model and predictions. There are three ways[^1]
to include more predictors:

-   **Backward Selection**: We would typically start with a **maximal
    model**[^2] and progressively simplify the model by knocking off
...
We will do the first two in the other tutorials (see above
...
other predictor/features as well. Mixed Selection we will leave for a
...
simple command `lm` to execute an Linear Model: Note the familiar
`formula` of stating the variables: ( $y \sim x$; where $y$ = target,
$x$ = predictor)
...
*standard errors* and *p.values*. And we see the fitted values of
`cmedv` for the existing `rm`; these values will naturally line **on**
the straight-line depicting the model. We will examine this `augment`-ed
data more in the @sec-diagnostics.

```{r}
#| label: predictions with new data
new <- data.frame(rm = seq(3, 10)) # must be named "rm"
...
## {{< iconify flat-color-icons workflow >}} Multiple Regression
...
flowchart TD
    ...
    B --> |corrplot +  corrgram  + purrr + ggformula| C(C: Check Relationships)
...
[^2]: Michael Crawley, *The R Book, Third Edition 2023. Chapter 9.
