...
distributed within groups, no outliers, then `independent t-test`\
2. If unequal variance, `t-test` with `Welch correction` ( Welch's
t-test)\
3.If samples are not normal, then nonparametric test ( Wilcoxon with two
...
- both data sets are normally distributed.Small samples may be assumed
to be `t-distributed`\
