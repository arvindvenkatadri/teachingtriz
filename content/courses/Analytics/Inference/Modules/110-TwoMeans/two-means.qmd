...
%%| echo: false
flowchart TD
    A[Inference for Independent Means] -->|Check Assumptions|
    B[Normality: Shapiro-Wilk Test shapiro.test\n Variances: Fisher F-test var.test]
    B --> C{OK?}
    C -->|Yes, both\n Parametric| D[t.test]
    D <-->F[Linear Model\n Method] 
...
(test) in the MathAnxiety dataset?
:::
...
of the data, **given** the Assumption that they are normally
distributed.
:::
...
Girls and Boys in the dataset `MathAnxiety`. So accordingly:
...
classical `wilcox.test`[Type `help(wilcox.test)` in your
...
test*[^5]! We will now use this philosophy to develop a technique that
allows us to mechanize several *Statistical Models* in that way, with
nearly identical code.

We **pretend** that `Gender` has not *effect* on the `AMAS` anxiety
...
Let us plot frequency distribution and Q-Q plots[^6] for both variables.
...
Note how the **Qual** variable was used here in Linear Regression! The
`gender` variable was treated as a binary "dummy" variable[^8].
:::

### Using the Permutation Test

For the specific data at hand, we need to shuffle the records between
`Semifinal` and `Final` on a per Swimmer basis and take the
`test statistic` (difference between the two swim records for *each*
swimmer). Another way to look at this is to take the differences between
`Semifinal` and `Final` scores and *shuffle the differences to either
polarity*. We will follow this method in the code below:
...
We have learnt how to perform inference for independent means. We have
looked at the conditions that make the regular `t.test` possible, and
learnt what to do if the conditions of normality and equal variance are
not met. We have also looked at how these tests can be understood as
...
[^5]: <https://allendowney.blogspot.com/2016/06/there-is-still-only-one-test.html>

[^6]: <https://stats.stackexchange.com/questions/92374/testing-large-dataset-for-normality-how-and-is-it-reliable>

[^7]: <https://stats.stackexchange.com/q/113337>
