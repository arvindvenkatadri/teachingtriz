...
#####
# For formatting numbers for good readability
...
`resampledata`. Here we have "anxiety" scores for boys and girls, for different components of mathematics. 

## {{< iconify carbon chart-3d >}} Inspecting and Charting Data

```{r}
#| label: Data Case Study 1
...
We have ~600 data entries, and with 4 Quant variables; `Age`,`AMAS`, `RCMAS`, and `AMAS`; and two Qual variables, `Gender` and `Grade`. 
...
Let us complete a check for normality: the `shapiro.wilk` test
checks whether a Quant variable is from a normal distribution; the NULL
hypothesis is that the data *are* from a normal distribution. We will also look at Q-Q plots for both variables:
...
infer whether there is any difference in the mean anxiety score between Girls and Boys in the dataset `MathAnxiety`. So accordingly:
...
What would be the **test statistic** we would use? The **difference in means**. Is the observed difference in the means between the two groups
...
similar, we typically cannot use a parametric `t.test`. However, we can still examine the results:
...
The `p.value` is $0.001$ ! And the `Confidence Interval` does not straddle $0$. So the `t.test` gives us good reason to reject the Null Hypothesis that
the means are similar and that there **is** a significant difference between Boys and Girls when it comes to `AMAS` anxiety. But can we really believe this, given the non-normality of data?


### Using the Mann-Whitney Test

Since the data variables do not satisfy the assumption of being
*normally distributed*, and though the variances are similar,
we use the classical `wilcox.test`[Type `help(wilcox.test)` in your
Console.]{.aside}, which implements what we need here: the *Mann-Whitney U test*:[^3]
...
`Gender` variable was treated as a binary "dummy" variable[^6].
:::
...
test*[^7]! We will now use this philosophy to develop a technique that
...
::: callout-important
The "pretend" position is exactly the NULL Hypothesis!!
The "uncommon" part is the `p.value` under NULL!!
:::
...
Clearly the `observed_diff_amas` is much beyond anything we can
...
the YRBSS was conducted.[Type this in your console:
`help(yrbss)`]{.aside}
...
Let us plot frequency distribution and Q-Q plots[^4] for both variables.
...
we use the classical `wilcox.test`[Type `help(wilcox.test)` in your
Console.]{.aside}, which implements what we need here: the *Mann-Whitney
U test*:[^5]
...
Finally, consider the possible relationship between a highschooler's weight
...
We have learnt how to perform inference for indendentnmeans. We have looked
...
[^4]: <https://stats.stackexchange.com/questions/92374/testing-large-dataset-for-normality-how-and-is-it-reliable>

[^5]: <https://stats.stackexchange.com/q/113337>

[^6]: <https://en.wikipedia.org/wiki/Dummy_variable_(statistics)>

[^7]: <https://allendowney.blogspot.com/2016/06/there-is-still-only-one-test.html>
