...
Girls and Boys, in the **population** from which the dataset `MathAnxiety` has been drawn. So accordingly:
...
$$
mean(rank(AMAS_{Girls})) - mean(rank(AMAS_{Boys})) = \mu_1;\\
$$ 
...
MathAnxiety %>% 
  ...
  gf_refine(scale_x_discrete(breaks =  c("Boy", "Girl"), 
                             labels = c("Boy", "Girl"), 
                             guide = "prism_bracket")) %>% 
  gf_label(label = TeX(r"(\textbf{Slope} $mu_1 \neq 0$ )"), 500 ~ 1.5, 
...
that with the `obs`erved `diff`rence and compute the `p-value` and `confidence intervals`:
...
gf_histogram(~ diffmean, data = null_dist_mosaic) %>% 
  gf_vline(xintercept = obs_diff_mosaic, colour = "red", linewidth = 2)
