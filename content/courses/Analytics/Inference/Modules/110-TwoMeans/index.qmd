...
means**. Is the observed difference in the means between the two groups of scores non-zero? We use the `diffmean` function:
...
*normally distributed*, and though the variances are similar, we use the classical `wilcox.test` (Type `help(wilcox.test)` in your
Console.) which implements what we need here:  the *Mann-Whitney U test*:[^3]
...
MathAnxiety %>% 
  ...
  gf_refine(scale_x_discrete(breaks =  c("Boy", "Girl"), 
                             labels = c("Boy", "Girl"), 
                             guide = "prism_bracket")) %>% 
  gf_label(label = TeX(r"(\textbf{Slope} $beta_1 \neq 0$ )"), 500 ~ 1.5, 
...
the YRBSS was conducted.The `yrbss` dataset is part of the `openintro` package. Type this in your console: `help(yrbss)`.
...
We will complete a visual check for normality with plots, and since we cannot do a `shapiro.test` we can use the Anderson-Darling test.
...
What would be the **test statistic** we would use? The **difference in
means**. Is the observed difference in the means between the two groups of scores non-zero? We use the `diffmean` function, from `mosaic`:
...
we use the classical `wilcox.test`, which implements what we need here: the *Mann-Whitney U test*,
...
Note how the **Qual** variable was used here in Linear Regression `lm()`! The `gender` variable was treated as a binary "dummy" variable[^8].
:::
...
null_dist %>%
  infer::get_confidence_interval(point_estimate = obs_diff_infer, 
                                 level = 0.95)
...
Clearly there is a serious effect of Physical Exercise on the body weights of students in this dataset.
...
manifestations of the *linear model*, with data and sign-ranked data.\
