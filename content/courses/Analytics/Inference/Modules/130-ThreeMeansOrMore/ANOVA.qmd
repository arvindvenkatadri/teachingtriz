...
Freedom]{style="background-color: yellow;"}. With $k = 2$ levels in the
...
The `F-stat` is compared with a **critical value** of the F-statistic,`F_crit`
which is computed using the formula for the f-distribution in R. As with
our hypothesis tests, we set the significance level to $\alpha = 0.95$, but here with the Bonferroni correction, and quote the two relevant degrees of freedom as parameters to `qf()` which computes the critical F value as a **quartile**:
