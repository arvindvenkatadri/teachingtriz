...
numbers for our `frogs` dataset and check:
...
::: column-margin
```{r}
#| label: Calculating SSE Error Sum of Squares
#| echo: false
frogs_plot <- frogs_long %>% 
  arrange(TempFac) %>% 
           rowid_to_column(var = "index")
...
```{r}
library(ggstatsplot)
frogs_long |> ggstatsplot::ggbetweenstats(
  x = TempFac, y = Time,
...
package [^7], which is based on the methods discussed in Judd et al.
@sec--references.
