...
abstract: "Using Randomization Tests to check the significance of a single Mean"
order: 100
categories:
- t.test
- Linear Model
...
underlying population. Depending upon the nature of the (single) variable, the test that can be used are as follows:
...
are made equal to zero in the linear model for a **single variable** !! The NULL Hypothesis therefore is:
...
when the number of observations $n >=50$. Lindoloev has a [simulation
on this.](https://lindeloev.github.io/tests-as-linear/simulations/simulate_wilcoxon.html).
