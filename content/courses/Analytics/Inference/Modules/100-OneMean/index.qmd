...
(In my opinion however, the code with `infer` is a little too verbose for us now, and does not offer quite the detailed, but quick insight that the `mosaic` package does). So let us stick with `mosaic` for now. We will do a permutation test first, and then a bootstrap test. 
