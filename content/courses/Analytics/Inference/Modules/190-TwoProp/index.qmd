...
`chisq` test: We will calculate the `observed-chi-squared` value, and
...
How do we know [what scores to expect]{.bg-lightred .black} if there was
...
>     with mean = $E_{ij}$ and as they are Poisson, their variance is
>     also $E_{ij}$.\
> -   Asymptotically the Poisson distribution approaches the normal
>     distribution with mean = $E_{ij}$ and *standard deviation* with
