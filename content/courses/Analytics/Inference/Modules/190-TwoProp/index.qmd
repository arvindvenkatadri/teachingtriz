...
All right, what of all this? How did this $X^2$ distribution come from? Here is a lovely, brief explanation from this [StackOverflow Post](https://stats.stackexchange.com/a/303548):
...
> - Asymptotically the Poisson distribution approaches the normal distribution with mean $E_{ij}$ and standard deviation with mean $\sqrt(E_{ij})$ so, asymptotically
$\frac{(X_{ij} - E_{ij})}{\sqrt(E_{ij})}$ is approximately standard normal.\
