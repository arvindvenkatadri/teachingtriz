...
::: callout-warning
In much the same way that the sample-means are distributed around the `pop_mean`, it appears that the sample-sds are also distributed around the `pop_sd`! However, the sample-sd distribution is **not Gaussian**, and the sample_sd is also **not an unbiased estimate** for the `pop_sd`. However, when the sample size $n$ is large ($n >= 30$), the sample-sd distribution approximates the Gaussian, and the bias is small. 
:::
